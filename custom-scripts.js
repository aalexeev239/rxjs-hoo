    ...
    ***************************************************************************** */
    ...
    function codeInit() {
        ...
        const request = value => {
            ...
            return rxjs.of(randomResponse).pipe(delay(randomTimeout));
        };
        ...
        const source$ = rxjs.interval(1200).pipe(
            take(3),
            map(v => v + 1)
        );
        ...
        function subInSub() {
            rxjs.of(1, 2, 3)
                .pipe(
                    map(value => rxjs.from(mapping(value)))
                )
        ...
        function iffyRecipe() {
            const source$ = rxjs.interval(1200).pipe(take(3), map(v => v + 1));
            let flag = true;
            ...
            source$
                .pipe(
                    switchMap(value => {
                        ...
                        return flag ? request(value) : rxjs.EMPTY
                    })
                )
        ...
        function windowOperator() {
            const sourceInterval$ = rxjs.interval(250).pipe(take(20));
            const windowInterval$ = rxjs.interval(1000);

            sourceInterval$
                .pipe(
                    window$1(windowInterval$),
        ...
        function groupByOperator() {
            ...
            rxjs.interval(1000).pipe(take(4), map(x => people[x]))
                .pipe(
